bootstraps <- pbreplicate(
  1000,
  bootstrapfnc(
    data = data_for_analysis,
    formula1 = paste(
      "outcome ~ ns(cumPAC, knots=quantile(cumPAC, probs=c(0.35,0.65)), Boundary.knots=quantile(cumPAC, probs=c(0.05,0.95)))*
    ...
    ns(VISIT, knots=c(6,12,24,36), Boundary.knots=c(1,42))",
    formula_baseline1 = paste(
      "outcome ~ ns(cumPAC, knots=quantile(cumPAC, probs=c(0.35,0.65)), Boundary.knots=quantile(cumPAC, probs=c(0.05,0.95)))*
      ns(VISIT, knots=c(6,12,24,36), Boundary.knots=c(1,42)) +",
      all_baseline_vars
    ),
    formula2 = paste(
      "outcome ~ ns(PAC, knots=quantile(PAC, probs=c(0.35,0.65)), Boundary.knots=quantile(PAC, probs=c(0.05,0.95)))*
      ns(VISIT, knots=c(6,12,24,36), Boundary.knots=c(1,42)) +
    ...
    formula3 = paste(
      "outcome ~ ns(cumPAC_year, knots=quantile(cumPAC_year, probs=c(0.35,0.65)), Boundary.knots=quantile(cumPAC_year, probs=c(0.05,0.95)))*
      ns(VISIT, knots=c(6,12,24,36), Boundary.knots=c(1,42)) +
